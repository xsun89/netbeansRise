...
//     updated by Xin. June 26, 2014 for using new Animal info data model
//----------------------------------------------------------------------------------
...
if (animalInformation != null) {
    ...
    for (var i = 1; i <= countAnimals; i++) {
        ...
        if (approvedAnimalsUsed != null) {
            var approvedAnimalsUsedItem = approvedAnimalsUsed.query("customAttributes.Species.ID='" + species.ID + "'");
            ...
            if (approvedAnimalsUsedItem != null) {
                ...
                if (approvedAnimalsUsedItemElm.count() > 0) {
                    histAnimalInfoMap[species.ID] = 1;
                    var totalNumAnimalRequestPerYear = numAnimalRequestPerYear + animalInfoMap[species.ID];
                    animalInfoMap[species.ID] = totalNumAnimalRequestPerYear;
                    approvedAnimalsUsedItemElm(1).setQualifiedAttribute("customAttributes.NumberOfAnimalsApproved", totalNumAnimalRequestPerYear);
    ...
    //approvedAnimalsUsed = targetEntity.getQualifiedAttribute("customAttributes.ApprovedAnimalsUsed");
    if (approvedAnimalsUsed != null) {
        var approvedAnimalsUsedElem = approvedAnimalsUsed.elements();
        for (var i = 1; i <= approvedAnimalsUsedElem.count(); i++) {
            var species = approvedAnimalsUsedElem(i).getQualifiedAttribute("customAttributes.Species");
            var hisAnimalUsedRcdDate = approvedAnimalsUsedElem(i).getQualifiedAttribute("customAttributes.HistoricAnimalUsedRecordDate");
            if(histAnimalInfoMap[species.ID] == null  || histAnimalInfoMap[species.ID] == 0)
            {
                approvedAnimalsUsedElem(i).setQualifiedAttribute("customAttributes.HistoricAnimalUsedRecord", true);
                if(histAnimalInfoMap[species.ID] == null && hisAnimalUsedRcdDate == null)
                    approvedAnimalsUsedElem(i).setQualifiedAttribute("customAttributes.HistoricAnimalUsedRecordDate", today);
            }else if(histAnimalInfoMap[species.ID] == 1) {
                approvedAnimalsUsedElem(i).setQualifiedAttribute("customAttributes.HistoricAnimalUsedRecord", false);
                approvedAnimalsUsedElem(i).setQualifiedAttribute("customAttributes.HistoricAnimalUsedRecordDate", null);
            }
        }
    }
}
