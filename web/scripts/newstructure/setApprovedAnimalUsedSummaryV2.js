...
function setApprovedAnimalUsedSummary() {
    try {
        ...
        var protArray = sessionContext.getContextObject("currentJsonObj");
        for (var i = 0; i < protArray.length; i++) {
            var jsonString = protArray[i];
            ...
            if (AnimalsUsed != 0 || jsonObj.num == "0") {
                ...
                if ((AnimalsUsed >= 0) && (NumberOfAnimalsUsed + AnimalsUsed > ApprovedAnimals)) {
                    ...
                    /*
                     ...
                     var Subject = "eSirius data pull for" + currentDate + " Overage Exists. Too many animals ordered. Verify that the approved animals in RISe match with eSirius for protocol " + this.ID;
                    ...
                    /*
                     ...
                     var Cc = "";
                     var From = "david.yeung@ors.ubc.ca";
                     var Subject = "eSirius data pull for" + currentDate + " Underage Exists. Too few animals ordered. Verify that the approved animals in RISe match with eSirius for protocol " + this.ID;
                     var HTMLBody = "Automatic generated email to transfer data from eSirius to RISe. " + e.description;
                     var objMail = CustomUtils.sendEmailCDO(To, Cc, null, From, Subject, HTMLBody, null, null);
