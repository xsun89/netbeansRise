function handleSubmit(sch, formEntity)
{
    try
    {
        ...
        if (persons.count()>0)
        {
            ...
            // Added by DY (apr 05) *****************************
            var RISeUserName = persons(1).firstname;
            RISeUserName += " ";
            RISeUserName += persons(1).lastname;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            if (CWLUserName == RISeUserName)
            {
                ...
                sch.redirectClientBrowser("./Login_UserInfo");
            }
            else
            {
                ...
                sch.redirectClientBrowser("./Login_Confirmation");
            }
...
function prerender(sch)
{
    ...
    if (ticket==null)
    {
        ...
        {
            sch.redirectClientBrowser("https://www.auth.cwl.ubc.ca/auth/login?serviceName=webors");
        }
        return false; //not render the rest of HTML
    }
    ...
    if (webrID) //if RISe # exists in CWL
    {
        ...
        if (persons.count()>0)
        {
            var p = persons(1);
            wom.getSessionContext.putContextObject("RISePerson", persons(1), true);
            var RISeUserName = p.firstname;
            RISeUserName += " ";
            RISeUserName += p.lastname;
            wom.getSessionContext.putContextObject("RISeUserName", RISeUserName, true);
